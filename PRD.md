...
A focused task management and team coordination tool with kanban boards, workload balancing, calendar views, commenting system, role-based permissions, and robust data management. Users manage tasks through drag-and-drop, balance workloads with AI-powered suggestions, track deadlines via calendar, collaborate via comments, control access through a permission system, and maintain data integrity with validation and export/import features.

## Essential Features

**Workload Balancing**
- Functionality: Intelligent workload distribution system that analyzes team member capacity, active tasks, priority levels, and overdue tasks to calculate workload scores. Visual dashboard displays each member's workload with progress bars, color-coded status (Overloaded, High, Balanced, Low), and detailed statistics (active tasks, critical/high priority tasks, overdue items). Auto-balance feature suggests optimal task reassignments based on role matching, capacity, and workload scores. Shows unassigned tasks requiring distribution. Member detail view displays task breakdown by status with individual task cards.
- Purpose: Ensure fair task distribution across team members, prevent burnout from overloading, identify capacity for new work, and optimize team productivity through balanced workloads
- Trigger: Navigate to Workload tab
- Progression: View workload dashboard with summary cards (team members count, unassigned tasks, average workload) → See team members sorted by workload score → Each card shows member avatar, role badge, workload level badge, score with progress bar, task statistics (active, total, critical, high priority), and View Details button → Click Auto-Balance → Dialog shows AI-suggested reassignments with reasoning → Review suggestions showing task priority, current assignee (or unassigned), target assignee with avatars, and explanation of why the change balances workload → Apply changes or cancel → Toast confirms reassignments → Click View Details on member → See comprehensive breakdown with status distribution cards (To Do, In Progress, Review, Done) and list of active tasks with priority flags, status badges, and overdue indicators → Close dialog
- Success criteria: Workload scores calculate accurately using weighted formula (active tasks × 1, critical × 3, high × 2, overdue × 2.5), auto-balance suggests optimal reassignments based on role and capacity, unassigned tasks section displays properly, member detail view shows complete task breakdown, all animations smooth, color coding consistent with workload levels, progress bars accurately reflect relative workload
