import { Task, TeamMember } from './collaboration-data'
import { 
  Bug, 
  Sparkle, 
  MagnifyingGlass, 
...
export interface TaskTemplate {
  ...
  autoAssignRoles?: Array<'architect' | 'developer' | 'devops' | 'product'>
}
...
export const taskTemplates: TaskTemplate[] = [
  ...
  },
  {
    ...
    suggestedDueDays: 7,
    autoAssignRoles: ['developer', 'architect']
  },
  {
    id: 'code-review',
    name: 'Code Review',
    ...
    suggestedDueDays: 2,
    autoAssignRoles: ['architect', 'developer']
  },
  {
    id: 'documentation',
    name: 'Documentation',
    ...
    suggestedDueDays: 5,
    autoAssignRoles: ['product', 'developer']
  },
  {
    id: 'devops',
    name: 'DevOps Task',
  ...
  },
  {
    ...
    suggestedDueDays: 1,
    autoAssignRoles: ['devops', 'architect']
  },
  {
    id: 'testing',
    name: 'Testing',
    ...
    suggestedDueDays: 3,
    autoAssignRoles: ['developer', 'product']
  },
  {
    id: 'ui-design',
    name: 'UI/UX Design',
  ...
  },
  {
    ...
    suggestedDueDays: 3,
    autoAssignRoles: ['developer', 'devops']
  },
  {
    id: 'release',
    name: 'Release',
    ...
    suggestedDueDays: 7,
    autoAssignRoles: ['devops', 'product']
  }
]
...
export function getAutoAssignedMember(
  template: TaskTemplate,
  teamMembers: TeamMember[]
): TeamMember | undefined {
  if (!template.autoAssignRoles || template.autoAssignRoles.length === 0) {
    return undefined
  }

  const availableMembers = teamMembers.filter(member => 
    member.isOnline && 
    template.autoAssignRoles?.includes(member.role)
  )

  if (availableMembers.length === 0) {
    const allMatchingRoles = teamMembers.filter(member =>
      template.autoAssignRoles?.includes(member.role)
    )
    return allMatchingRoles[0]
  }

  const memberTaskCounts = availableMembers.map(member => ({
    member,
    count: 0
  }))

  const sortedMembers = memberTaskCounts.sort((a, b) => a.count - b.count)
