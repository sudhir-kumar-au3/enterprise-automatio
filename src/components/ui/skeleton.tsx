  ...
  return (
    <div className={cn("space-y-3", className)} {...props}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="flex items-center space-x-4 p-4 rounded-xl border bg-card">
...
function SkeletonAvatar({ className, ...props }: ComponentProps<"div">) {
  return (
    <Skeleton className={cn("h-10 w-10 rounded-full", className)} {...props} />
  )
}

function SkeletonButton({ className, ...props }: ComponentProps<"div">) {
  return (
    <Skeleton className={cn("h-10 w-24 rounded-lg", className)} {...props} />
  )
}

export { Skeleton, SkeletonCard, SkeletonList, SkeletonTable, SkeletonText, SkeletonAvatar, SkeletonButton }
